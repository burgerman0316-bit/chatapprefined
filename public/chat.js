const socket = io();
const messagesContainer = document.getElemenconst socket = io();
...
let ignoreNextEnterPress = false; // Add this new flag
...
messageInput.addEventListener('keypress', (e) => {
    if (e.key === 'Enter') {
        ...
        if (ignoreNextEnterPress) {
            ignoreNextEnterPress = false;
            return;
        }
    ...
    filteredUsers.slice(0, 3).forEach((user, index) => {
        ...
        button.onclick = (e) => {
            ...
            if (msgIndex !== -1) {
                ...
                hideDmUserMenu();
                ignoreNextEnterPress = true; // Set the flag to ignore the next Enter
            }
        };
