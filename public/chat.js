// chat.js - Fixed version addressing chat switching, machine gun, and anonymous names
...
// FIX #1: Only switch context if different from current
function switchChatContext(contextId) {
if (!isAdmin && contextId === ADMIN_CHAT_ID) return;

// FIXED: Don't clear if we're already on this context
if (currentChatContext === contextId) {
    return;
}
...
// FIX #2: Handle admin chat messages properly
