...
socket.emit('check_ban_status', googleId);
...
if (msg.type === 'system') {
    ...
    if (msg.isAdmin || isAdmin) {
        item.classList.add('admin-msg');
    }
    
    const nameDisplay = (msg.username === 'You' && msg.isPrivate) ? 'You' : msg.username;
...
if (content.startsWith('/')) {
    ...
    if (command === '/msg') {
        ...
        if (banMatch) {
            ...
            if (targetSocketId) {
                // Need to get Google ID - simplified version
                appendMessage({ username: 'System', content: 'Please use the Admin Panel to ban users.', timestamp: new Date(), type: 'system' });
            } else {
        ...
        appendMessage({ username: 'System', content: 'Please use the Admin Panel to unban users.', timestamp: new Date(), type: 'system' });
    }
...
// 5. Admin: Kick User Confirmation Click Handler
